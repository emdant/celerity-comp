...
mkdir -p features-sycl-bench
mkdir -p sycl-bench-features-count
mkdir -p sycl-bench-features-normalized
...
for file in *.bc; do
    ...
    echo "--- extracting features from sycl-bench/$file ---"
    opt -load-pass-plugin ../libfeature_pass.so  --passes="print<feature>" -disable-output $file 1>> "../features-sycl-bench/$name.temp"
done

cd ../features-sycl-bench
for tempfile in *.temp; do
  name="${tempfile%.*}"

  features_count="../sycl-bench-features-count/${name}_features.csv"
  features_norm="../sycl-bench-features-normalized/${name}_features.csv"
