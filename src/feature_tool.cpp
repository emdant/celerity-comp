...
cl::opt<FeatureSetOptions> FSet("fset", cl::desc("Specify the feature set:"),
    cl::values(clEnumVal(fan19, "Default feature set for GPU used in [Fan et al. ICPP 19]"), clEnumVal(grewe13, "Feature set used in pGrewe et al. 13]"),
...
std::unique_ptr<Module> load_module(LLVMContext& context, const std::string& fileName, bool verbose) {
	SMDiagnostic error;
	if(verbose) cout << "loading module from file" << fileName << endl;
