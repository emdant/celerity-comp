		...
		for(Instruction& inst : bb) {
			...
			if(auto* ci = llvm::dyn_cast<llvm::CallInst>(&inst)) { // we assume direct call here
				...
				if(ci->getNumOperands() == 1) {
					if(fnd.rfind("get_num_sub_groups", 0) == 0) invariants[InvariantType::nsg] = inst_ptr;
					if(fnd.rfind("get_sub_group_size", 0) == 0) invariants[InvariantType::sgs] = inst_ptr;
					if(fnd.rfind("get_max_sub_group_size", 0) == 0) invariants[InvariantType::msgs] = inst_ptr;
				}
			}
		}
	}
